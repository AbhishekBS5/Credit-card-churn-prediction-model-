...
from imblearn.over_sampling import SMOTEwarnings.filterwarnings("ignore")
...
def load_data(path: str) -> pd.DataFrame:
 ...
 df = pd.read_csv(path)
 # The Kaggle release ships two trailing Naive Bayes classifier columns
 # that are not meant to be used as features. Drop them if present.
...
def chi_square_feature_ranking(df: pd.DataFrame, target_col: str, top_n: int = 10) -> list:"""
 ...
 Continuous features are binned into quartiles first so the test applies cleanly.
 Returns the top_n feature names, strongest first.
 """
 ...
 # Column had too few unique bins to form a valid contingency table
 continue
...
def train_and_evaluate(X_train, X_test, y_train, y_test):
 ...
 # Correct 16% minority class imbalance with SMOTE, applied to training data only
 smote = SMOTE(random_state=RANDOM_STATE)
 X_train_res, y_train_res = smote.fit_resample(X_train_scaled, y_train)
 results = {}
 rf_params = {"n_estimators": [200, 400], "max_depth": [8, 12, None]}rf_grid = GridSearchCV(
